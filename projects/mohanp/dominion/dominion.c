...
int sea_hagEffect(struct gameState *state, int currentPlayer){
	...
	if (i != currentPlayer){
    //Mutant: incrementing the discardCount before the reduction of the deckCount
...
int remodelEffect(struct gameState *state, int currentPlayer,int handPos, int choice1, int choice2,int j) {
	  ...
	  if (state->hand[currentPlayer][i] == j)
	    {
	      ...
	      //Mutant instead of a break statement a continue statement
	      //break;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case remodel:
      remodelEffect(state,currentPlayer, handPos, choice1, choice2, j);
      return 0;
